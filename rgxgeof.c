/* rgxgeof ...  regex for geo file 
 * Yes of course these are xml and can be parsed with libxml, but this is an exercise
 * taken from dem5.c
...
int main(int argc, char **argv)
{
    ...
    /* now we can set up our dataholder */
    ...
    for(i=0;i<sbasz;++i) 
        if(GPXYES)
            dholder[i]=calloc(dhbuf, sizeof(gpx_t));
#if defined DBG
